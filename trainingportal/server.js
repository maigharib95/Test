...
const requestsSol = require(path.join(__dirname, 'requests'));
...
var instructorHtml = fs.readFileSync(path.join(__dirname, 'static/instructor.html'), 'utf8');
...
const approvedUsersPath = util.getDataDir() + "/static/approvedUsers.json";
const requestSolutionsPath = util.getDataDir() + "/static/requestSolutions.json";
...
app.post('/', function (req, res) {
  // add approved data
  var approvedUsers = req.body;
  if ((approvedUsers.length != null) || approvedUsers.length != "0") {
    console.log("Approved Length : " + approvedUsers.length);
    fs.writeFileSync(approvedUsersPath, JSON.stringify(approvedUsers), 'utf-8', function (err) {
      if (err) throw err
      console.log('Done!');
    });
    console.log("approved Users : " + JSON.stringify(approvedUsers));
  }
});
...
app.get('/challenges/:moduleId/level', async (req, res) => {
  ...
  // levels accomplished
  //console.log("Levels : " + JSON.stringify(userLevelForModule));
  res.send({ "level": userLevelForModule });
});

app.get('/request/:challengeId', (req, res) => {
  var challengeId = req.params.challengeId;
  console.log("Challenge ID : " + JSON.stringify(challengeId));
  requestsSol.createRequestSolution(challengeId);
});

app.get('/challenges/solutions/:challengeId', (req, res) => {
  var solutionHtml = "";
  var approvedFile = fs.readFileSync(approvedUsersPath, 'utf-8');
  var arrayApproved = JSON.parse(approvedFile);
  var challengeId = req.params.challengeId;
  var localStorage = new LocalStorage('./scratch');
  var loggedUser = localStorage.getItem('loggedInUser');
  for (var i = 0; i < arrayApproved.approvedUsers.length; i++) {
    if ((arrayApproved.approvedUsers[i].name == loggedUser) && (arrayApproved.approvedUsers[i].challengeId == challengeId)) {
      //console.log("Challenge ID : "+ JSON.stringify(challengeId));
      ...
      solutionHtml = challenges.getSolution(challengeId);
      //console.log("Solutions : " + JSON.stringify(solutionHtml));
    }
  }
  res.send(solutionHtml);
});

// Get Description for Challenges
app.get('/challenges/descriptions/:challengeId', (req, res) => {
  ...
  //console.log("Description : " + JSON.stringify(descriptionHtml));
...
app.get('/api/user', (req, res) => {
  db.fetchChallengeEntriesForUser(req.user, function () {
    ...
    //console.log("Passed Challenges : "+ JSON.stringify(passedChallenges));
...
var returnListWithChallengeNames = function (res, list) {
  ...
  //console.log("Challenges Name : "+ JSON.stringify(list));
  res.send(list);
};
